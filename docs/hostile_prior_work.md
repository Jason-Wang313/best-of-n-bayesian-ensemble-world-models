...
| MOPO | It already penalizes model uncertainty in offline MBRL. | MOPO is a policy-optimization algorithm. The current contribution is a diagnostic and repair for posterior-tail candidate selection at inference time. |
...
| MOBILE | It uses model Bellman inconsistency as an uncertainty penalty. | MOBILE is a training-time offline-RL penalty. This repo asks how to aggregate ensemble rollouts when candidate selection consumes posterior uncertainty. |
...
| RWM-O/RWM-U | It is an uncertainty-aware robotic world model in the exact application family. | RWM-U uses uncertainty for offline policy robustness. It does not present a controlled posterior-sample tail-selection failure. |
